...
import pandas as pd
import torch
from pipeline.EdgeClassification import FeatureRegistry, ProvidedSplitsTask, TNNTrainConfig, TaskHooks, run_pipeline_for_task
...
def _make_task(
    ...
    FeatureRegistry.register_custom_features(
        ["node_type", *list(proj_keys), *list(context_keys)],
        feature_types=["node"] * (1 + len(proj_keys)) + ["edge"] * len(context_keys),
    )
    hooks = TaskHooks(
        label_fn=None,
        feature_set=[
